...
export default function OnboardingPage() {
  ...
  const [brandBootstrapWarning, setBrandBootstrapWarning] = useState(false)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Bootstrap with the company name and mark onboarding complete
      await client.accounts.bootstrap(accountName.trim(), { onboardingCompleted: true })

      // If a website URL was provided, kick off brand bootstrap
      if (websiteUrl.trim()) {
        try {
          await client.brandKit.bootstrapFromUrl(websiteUrl.trim())
        } catch (bootstrapErr) {
          // Non-fatal — log and show a warning, but still proceed to dashboard
          console.warn('Brand bootstrap failed:', bootstrapErr)
          setBrandBootstrapWarning(true)
        }
      }
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md">
        {/* Card */}
        <div className="bg-white/85 backdrop-blur-md rounded-xl shadow-2xl p-8">
          {/* Logo */}
          <div className="flex justify-center mb-6">
            <div className="relative overflow-hidden" style={{ width: 240, aspectRatio: '3/1' }}>
              <Image
                src="/logo.png"
                alt="CounterPromo"
                fill
          ...
          {step === 1 ? (
            <form onSubmit={handleStep1} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm text-slate-900 placeholder-slate-400 bg-white/80 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent"
                />
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {brandBootstrapWarning && (
                <p className="text-sm text-amber-700 bg-amber-50 px-3 py-2 rounded-md">
                  We couldn&apos;t scrape your website right now — you can retry from the Branding page after setup.
                </p>
              )}
